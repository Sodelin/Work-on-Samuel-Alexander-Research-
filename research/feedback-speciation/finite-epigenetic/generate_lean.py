...
d=json.loads((root/"count-certificates.json").read_text())
template=r"""import FiniteFixation
...
def certificateNum (epi : Bool) (x : ℕ) : ℕ :=
  (if epi then __EPICERT__ else __GENCERT__).getD x 0
...
"""
for tag,key in [("__EPICERT__","epigenetic"),("__GENCERT__","genetic")]:
 template=template.replace(tag,str(d[key]["certificateNumerators"]))
(root/"FiniteEpigenetic.lean").write_text(template,encoding="utf-8")
print("wrote FiniteEpigenetic.lean",len(template))
