...
1. Every $`v\ge 1`$ has finite $`L(v)`$ and $`3 L(v)\le 8v-1`$.
2. For every integer $`n\ge 0`$,
   $`L(3\cdot 2^n-1)=8\cdot 2^n-3`$.
3. Equality in $`3 L(v)\le 8v-1`$ occurs exactly when
   $`v=3\cdot 2^n-1`$ for an integer $`n\ge 0`$.
...
```math
\begin{aligned}
f_b(x)&=\begin{cases}x+1,&t(x+1)=b,\\x+2,&\text{otherwise},\end{cases}\\
X_0(v)&=v,\\
X_{k+1}(v)&=f_{t(k)}(X_k(v)).
\end{aligned}
```
For every $`v\ge 1`$, the complete matching frontier at depth $`k`$ is

```math
[X_k(v),X_k(v+1)).
```
Each $`f_b`$ is nondecreasing. Hence $`X_k(u)\le X_k(v)`$ whenever $`u\le v`$.
...
X_k(0)=2k
```
by induction. Call this the baseline. Monotonicity gives $`X_k(v)\ge 2k`$ for
every $`v\ge 0`$. Once a trajectory meets the baseline it remains there.

We repeatedly use the following binary-block identity. If $`r`$ is a power
of two and $`0\le i<r`$, then

```math
t(ar+i)=t(a)\oplus t(i).
```
...
Fix a power of two $`q\ge 1`$ and put $`T=8q`$. Suppose that at time $`4q`$ a
...
\end{aligned}
```
**Proof.** For each $`r=q,q/2,\ldots,1`$, consider the state

```math
k=T-4r,\qquad x=2T-6r-1.
```
...
Write $`M=\frac{T}{r}=2^s`$, where $`s\ge 3`$. In the first run, for $`0\le i<r`$, the target
and the inspected source vertex have bits

```math
\begin{aligned}
\text{target: }&t((M-4)r+i),\\
\text{source: }&t(x+2i+1)\\
&=t(2((M-3)r+i))\\
&=t((M-3)r+i).
\end{aligned}
...
In the second run, for $`0\le j<r`$, the inspected source bit and target bit
are respectively

```math
t((2M-4)r+j),\qquad t((M-3)r+j).
```
...
\text{time}&=T-2r,\\
\text{vertex}&=2T-3r-1,\\
\text{offset above the baseline}&=r-1.
\end{aligned}
```
...
at step $`0\le i<4q`$, this proposed run compares $`t(6q+i)`$ with $`t(i)`$. The
...
**Run I.** For $`0\le i<2r`$, the inspected source bit is
...
$`t(10r+1)=1`$. For $`r=1`$, the latter is $`t(11)=1`$. For $`r\ge 2`$, the block
identity gives $`t(10r+1)=t(10) \oplus t(1)=0 \oplus 1=1`$. Thus this advance is 1.

**Run III.** For $`0\le i<r-1`$, put $`j=1+i`$, so $`1\le j<r`$. The source bit is
...
For any integer $`v\ge 2`$, choose the power of two $`q`$ such that

```math
3q\le v+1<6q.
```
Such a $`q`$ exists by taking the largest dyadic $`q`$ with $`3q\le v+1`$.
Since the quantities are integers, $`v+1\le 6q-1`$. At time $`K=8q-2`$,
monotonicity, the baseline identity, and Lemma 2 give

```math
2K=X_K(0)\le X_K(v)\le X_K(v+1)\le X_K(6q-1)=2K.
```
Both frontier boundaries coincide, so the interval theorem proves
finiteness and $`L(v)\le K-1=8q-3`$. Moreover $`v\ge 3q-1`$, and consequently

```math
3L(v)\le24q-9\le8v-1.
```
This proves the claimed bound for every $`v\ge 1`$.

## The equality family

Fix a power of two $`q\ge 1`$, put $`v=3q-1`$, and set $`T=8q`$.
...
For $`q\ge 2`$, apply Lemma 2 with power of two $`q/2`$: its starting vertex
...
L(3q-1)=T-3=8q-3.
```
Substituting $`q=2^n`$ gives the requested equality for every integer $`n\ge 0`$.

## Exact equality cases and optimality

For every $`v\ge 2`$, the global proof chose a dyadic $`q`$ with
$`3q-1\le v\le 6q-2`$ and proved
...
```math
\frac{L(3q-1)}{3q-1}=\frac{8q-3}{3q-1}\longrightarrow\frac{8}{3}
```
as dyadic $`q`$ tends to infinity. Therefore $`\frac{8}{3}`$ is the optimal leading
coefficient: no inequality $`L(v)\le c v+C`$ with fixed $`c<\frac{8}{3}`$ and fixed
finite $`C`$ can hold for all starts. For coefficient $`\frac{8}{3}`$, the additive
term $`-\frac{1}{3}`$ in the displayed global bound is also attained by every member
...
formula. Define $`H(v)`$ as the first $`k\ge 0`$ with $`X_k(v)=2k`$. Then

```math
\begin{aligned}
H(0)&=0,\\
H(1)&=H(2)=2,\\
H(v)&=8q-2\quad\text{for }3q\le v\le6q-1,\quad q\text{ a power of two}.
\end{aligned}
```
...
$`0\le n\le 8`$, 8,140 advance comparisons, and 256 bounded $`H(v)`$ checks, and
