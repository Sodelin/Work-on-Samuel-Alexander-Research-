...
$`1-t(w)`$. There is no edge $`0 \to 1`$. Let $`L(v)`$ count the maximum number of
initial target labels matched from $`v`$; in particular, $`L(0)=1`$.

## Candidate closed form

Set $`n=\left\lfloor v/2\right\rfloor`$ and write uniquely

$`n+1 = 2^r (2h+1)`$, with $`q=2^r`$, $`T=t(h)`$, and $`U=t(h+1)`$.

Let $`A(h)=1`$ exactly when $`h\equiv1\pmod{4}`$, $`t(\lfloor h/4\rfloor)=0`$, and
...
$`X=\frac{L(4n+1)-t(n)}{2}`$, $`F=L(4n+2)`$, $`C=L(4n+3)`$.
