...
**Question.** For an aperiodic binary word `s`, define `d_vertex(s)` to be the
least uniform child cap among eligible populations avoiding `s` in which gender
belongs permanently to each vertex. Is `d_vertex(s)=2` for every such `s`, or
...
is exactly `k(k+1)/2`. Is the graph, outside a finite prefix in birth order,
...
`C_N >= k(k+1)/2` after all roots, and proves eventual full indegree and
...
the subsequences `L(2^e n+r)`, for `e>=0` and `0<=r<2^e`, is finitely generated.
...
and the 2-adic valuation of `floor(v/2)+1`. Its [independent finite
...
already spell tails `t(a),t(a+1),...` for `a>=2`.

**Why pursue it.** The original sharp theorem has a privileged phase-zero
baseline. A phase theorem would tell us which part of its `8/3` constant comes
...
proves `3L_a(v)<=8v+5a-1` for positive starts and actual long dyadic witnesses
with their starts in a bounded interval. [RealBridges](real/RealBridges.lean)
proves that the leading coefficient is exactly `8/3` for every fixed phase,
...
`3L_a(v)=8v+5a-1` holds exactly when `v=3*2^n-a-1` and `a<=2^n`, with
`L_a(v)=8*2^n-a-3`. The [formalization note](notes/PHASE-EXTREMAL-FORMALIZATION.md)
records the construction and uniqueness argument. Joint digit recurrences
remain open.

**First decisive test.** Adapt the exact interval checker to `row(s_a)` and
...
if `s(k+p)=s(k)` for `k<len`, then the explicit two-step path starting at
`2p-1` matches `len` edges. Thus very long periodic prefixes create very long
matches even when the full word is aperiodic. Aperiodicity alone supplies
finiteness, not a useful numerical rate.

**New checked theorem.** For every `f : Nat -> Nat`, there is an aperiodic
...
does the optimal leading coefficient of `L_s(v)` remain `8/3`? More explicitly,
does an edit-dependent constant `B` give `3L_s(v)<=8v+B` for all positive
starts, and is every smaller leading coefficient impossible? Upper stability
and lower sharpness are separate assertions.

**New checked theorem.** If `s(k)=t(k)` for every `k>=m`, then
`3L_s(v)<=8v+8m-1` at every positive start. Generic finite-path transport
...
[RealBridges](real/RealBridges.lean) excludes every coefficient below `8/3`,
...
**Why pursue it.** A positive answer would make `8/3` an invariant of an
...
potential `h` and bound `c` such that every permitted transition `q -> q'`
with displacement `d` satisfies `u·d <= c+h(q)-h(q')`. Summing along a lifeline
