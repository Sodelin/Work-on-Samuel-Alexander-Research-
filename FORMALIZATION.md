...
| [GeneralRootObstruction](lean/SamuelAlexanderResearch/GeneralRootObstruction.lean) | The first $`k`$ indices are roots, at least $`k`$ distinct roots exist, and whole-population common ancestry fails for $`k\ge2`$. | Simplicity and all required incoming labels are essential; the count is at least $`k`$, not exactly $`k`$. |
...
| [SharpThueMorse](lean/SamuelAlexanderResearch/SharpThueMorse.lean), [ThueMorseBits](lean/SamuelAlexanderResearch/ThueMorseBits.lean), [ThueMorseBound](lean/SamuelAlexanderResearch/ThueMorseBound.lean) | Actual bits and exact interval reachability imply $`3L(v)\le8v-1`$ for $`v\ge1`$, with equality exactly at $`v=3\cdot2^n-1`$, where $`L=8\cdot2^n-3`$. | Actual original edges, edge-count lengths, attained maxima, no unchecked trajectory premise. |
...
| [FiniteEditStability](lean/SamuelAlexanderResearch/FiniteEditStability.lean) | If $`s=t`$ from index $`m`$, actual finite matches transport with unchanged length and start displacement at most $`m`$; $`3L_s(v)\le8v+8m-1`$ for $`v\ge1`$, maxima exist, and arbitrarily late near-sharp witnesses exist. | $`m`$ bounds the edited initial segment. The exported displacement is at most $`m`$; $`\min(m,\ell)`$ is the suffix cutoff. |
| [PhaseShift](lean/SamuelAlexanderResearch/PhaseShift.lean) | For the graph rebuilt from $`t(k+a)`$, $`3L_a(v)\le8v+5a-1`$ for $`v\ge1`$ and actual dyadic lower witnesses exist in a specified start interval. | Both graph and target change. [PhaseExtremal](lean/SamuelAlexanderResearch/PhaseExtremal.lean) proves equality exactly when $`v=3\cdot2^n-a-1`$ and $`a\le2^n`$, with maximum $`8\cdot2^n-a-3`$; see its [scope note](notes/PHASE-EXTREMAL-FORMALIZATION.md). |
...
The avoiding graph has no $`0\to1`$ edge, path length counts edges, and the
