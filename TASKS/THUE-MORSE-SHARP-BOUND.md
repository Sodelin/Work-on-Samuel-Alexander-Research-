...
Let $`t(n)`$ be the parity of the binary digit sum of $`n`$. The vertices are the nonnegative integers. For each $`w\ge2`$, there are edges $`(w-1)\to w`$ labelled $`t(w)`$ and $`(w-2)\to w`$ labelled $`1-t(w)`$; there is no edge $`0\to1`$. For a start vertex $`v`$, a path of length $`\ell`$ matches when its edge at index $`k`$ is labelled $`t(k)`$ for every $`0\le k<\ell`$. Let $`L(v)`$ be the maximum matching length.

## Original questions, now resolved

1. Is $`3L(v)\le8v-1`$ for every integer $`v\ge1`$?
2. Is $`L(3\cdot2^n-1)=8\cdot2^n-3`$ for every integer $`n\ge0`$?
...
- State the exact theorem or counterexample, including all quantifiers, the missing $`0\to1`$ edge, and the target starting at $`t(0)`$.
