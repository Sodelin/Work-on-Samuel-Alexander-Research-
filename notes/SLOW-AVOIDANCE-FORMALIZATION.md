...
`MatchesPrefix s path len` checks the actual edge relation for every index `k < len`, reading target bit `s k` on the kth edge. Thus `len` counts edges, and each finite match starts at phase zero. The absence of an infinite match is separately proved for this same constructed word and this same graph. No finite experiment is used to establish either assertion.
...
- At stage `j+1`, positions `k < M_j` read the old word at `k % p_j`; positions `M_j ≤ k < 2M_j` are false; position `2M_j` is true. Later positions are outside the finite word and are immaterial.

Since `r_j ≥ 2`, `W_j` is a prefix of `W_{j+1}`. The lengths strictly increase and satisfy `p_j ≥ j+1`. The limit is therefore defined without an existential choice of an infinite extension:
...
Both positions `k` and `k+p_j` lie in the repeated block whenever `k < ℓ_j`. Their remainders modulo `p_j` are equal. Prefix coherence therefore gives

\[
s(k+p_j)=s(k)\qquad(k<\ell_j).
\]

`target_periodic_prefix` proves this identity. `QuantitativeAvoidance.periodic_prefix_gives_match` then supplies a matching path with `ℓ_j` edges starting at `v_j=2p_j−1`. Its explicit form is `path(k)=2k+v_j`, so it always takes a two-step edge. Because `p_j ≥ 1`,

\[
\ell_j\ge f(v_j)+1>f(v_j).
\]

The strictly increasing lengths `p_j` give strictly increasing `v_j`. The formal endpoints are `matchLength_exceeds`, `starts_strict`, and `finite_match_exceeds`. No monotonicity of `f` enters the argument.
