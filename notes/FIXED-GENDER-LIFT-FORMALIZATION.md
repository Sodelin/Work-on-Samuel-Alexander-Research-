...
For each `w ≥ 2`, the incoming edges are `(w−1) → w` with label `r(w)` and `(w−2) → w` with label `¬r(w)`; there is no edge `0 → 1`.
...
where `base x = x / 2` and `gender x` is the parity bit. Thus every lifted edge is labelled by the **fixed gender of its source**. In pair notation, `(u,a) → (w,b)` holds exactly when `E_s u w a`, independently of `b`.
...
At base zero the retained gender is `¬s(1)`. These identities handle the roots and the degree argument without exceptional unproved cases.
...
`Clean s` removes only `(0,s(1))` and `(1,¬s(1))`, the two isolated copies at root bases. It keeps both copies at every base at least two. `clean_roots_exactly` proves that its retained roots are the same two productive roots as above. `clean_avoider` proves the four-child population conditions, weakly connected specieslike status, aperiodic avoidance, and failure of inspecies minimality.
