...
satisfy `3a<8b`, some actual matching maximum obeys `a*v+c<b*ell`.
...
`s(k+p)=s(k)` for `k<len`, taking two-step edges from `2p-1` yields a match
of `len` edges. The row-odd formula verifies every actual edge and retains
the missing `0 -> 1` convention. Fully periodic targets give an infinite
