...
For a Boolean target s, the module defines r(2j)=s(j) and r(2j+1)=not s(j). An edge into w exists only for w >= 2. It comes from w-1 with label r(w), or from w-2 with the complementary label. Thus the missing edge 0 -> 1 is encoded, and the edge at path index k must spell s(k).

The formal proof establishes:

1. Every matching infinite path satisfies path(k) >= 2k.
2. The natural offset path(k)-2k never increases.
...
The positive theorem that every eventually periodic sequence occurs in every Alexander population is not formalized by this module. Nor does it formalize the finite-alphabet copy-lift, the Thue–Morse overlap-free theorem, or the sharp quantitative conjecture. Natural-number birthdate properties are checked separately; a real-valued birthdate specialization is outside this standard-library development.
