...
negative `P_s` witness inside the literal real model. Together with the positive
...
`ell <= (8/3)*v - 1/3`. `real_coefficient_optimal` proves that for every real
`c<8/3` and every real `C`, an actual positive-start maximum exceeds `c*v+C`.
...
`ell <= (8/3)*v + (8*m-1)/3` and
`ell <= (8/3)*v + (5*a-1)/3`, and the same real-coefficient optimality for
...
number** in the particular graph `P_s`. They do not measure growth against
