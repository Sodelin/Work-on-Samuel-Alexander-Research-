...
`lean/SamuelAlexanderResearch/PhaseExtremal.lean` proves the exact equality classification in the actual graph built from the shifted Thue–Morse target. Let `t` be the checked binary digit-parity sequence, put `s_a(k)=t(k+a)`, and let `L_a(v)` denote the maximum number of edges in a phase-zero `s_a`-matching path from `v` in `BinaryAvoidance.Edge s_a`. For every `a≥0` and `v≥1`, the maximum exists and
...
All matching statements use `BinaryAvoidance.Edge`, including its missing edge `0→1`. A phase shift changes both the target word and the graph constructed from it. These results do not concern reading a shifted target in the unchanged graph `P_t`.

`FiniteEditStability.IsMaximumPrefix s v len` means that an actual path function attains `len` matching edges from `v` and that every other attained matching length from `v` is at most `len`. It is not an assumed numerical bound or an interval-model substitute for actual paths.

`PhaseExtremal.shifted_maximum_exists` proves that every positive start has such a maximum and that it satisfies the displayed upper bound. The main endpoint `PhaseExtremal.shifted_extremal_iff` states, for `a v len : Nat` with `1≤v`,
...
For dyadic `q≥2`, the recursive construction is
...
The implementation builds actual finite paths using `Part`, `Segment`, and `joinPath`. `Part phase path len` checks each actual edge against `t(phase+i)`. One-step and two-step segments use the explicit functions `x+i` and `x+2*i`. Concatenation preserves their shared endpoint and all early vertices. Thus the proof constructs the run pattern directly; it does not postulate a path with those comparisons.
...
The last run uses `¬t(3q+i)=t(2q+i)` for `i<q+2`. Since `q≥2`, this range lies below `2q`, where the relevant high-bit comparisons are `(3,2)` and `(4,3)`. Every equality follows from the existing proved binary-block identity for the actual sequence `t`.
...
The tail begins exactly where the head ends and finishes at `16q−6`. Induction gives `extremal_path`. This endpoint records the exact start, length, terminal vertex, first `q` one-steps, and, for `q≥2`, the next two-step. It asserts a finite prefix of an actual path function; values of that function after the prefix are immaterial.

## Attaining shifted equality

For `a≤q`, take the constructed original extremal path and remove its first `a` edges. Those edges all have length one, so its vertex at time `a` is `3q−1+a`. The already checked phase transport subtracts `2a` from every remaining vertex. The resulting shifted path starts exactly at
...
Suppose a shifted matching path of length `len` starts at `v≥1` and satisfies `3len=8v+5a−1`. `PhaseShift.prepend_shifted_prefix` produces an original matching path of length `len+a`, starting at some `u` with `v≤u≤v+a`. The original sharp inequality then forces

\[
u=v+a,\qquad 3(len+a)=8u-1.
\]

`original_equality_dyadic` applies the original sharp equality classification to this actual finite path. Its maximality is derived from the saturated sharp bound, not assumed separately. Hence `u=3q−1` and `len+a=8q−3` for some dyadic `q`.
...
For `q≥2`, the unique extremal path's step at time `q` has length two. The prepended one-step prefix consequently cannot extend past time `q`, proving `a≤q`. For `q=1`, its first two steps are both one, but `u=2=v+a` and `v≥1` already force `a≤1=q`. This handles the base case without an incorrect uniform claim about the first non-one step.
