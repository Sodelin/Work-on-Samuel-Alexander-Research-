...
The binary edge-labelled construction `P_s` has the same underlying graph for
every target word `s`. Its vertices are `Nat`, with

```text
PsEdge u w  :=  2 <= w and (w = u + 1 or w = u + 2).
```

There is no edge `0 -> 1`. Labels do not enter the species predicates, so the
...
| --- | --- |
| `positive_reaches` | Every `u >= 1` reaches every `w > u`. |
| `zero_reaches` | Vertex `0` reaches every `w >= 2`. |
| `psDescendant_iff` | Strict reachability is exactly `u < w` and `2 <= w`. |
| `psNonDescendant_iff` | A non-descendant `w` is exactly one with `w <= u` or `w <= 1`. |
...
A matching path has `path(k) >= 2k`; the natural offset `path(k) - 2k` is
...
For `Cone r = {r} union {strict descendants of r}`, the module proves all four
properties and weak connectivity. In particular:

```text
C0 = Cone 0 = {0} union {v | 2 <= v}
C1 = Cone 1 = {v | 1 <= v}
```
