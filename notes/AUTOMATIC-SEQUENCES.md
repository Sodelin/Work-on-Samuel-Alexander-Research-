...
The classification manuscript's Section 5 considers its specific binary population $P_{s}$, built from a binary target sequence $s$. It states and argues that

$$
P_s\text{ realizes every infinite binary sequence}\iff s\text{ is eventually periodic}.
$$

Allouche, Rampersad, and Shallit give a proof of Honkala's theorem that eventual periodicity is decidable for a $k$-automatic sequence supplied by a finite automaton with output. Therefore, when the input for $s$ is such an automaton, one can decide whether the **particular constructed graph** $P_{s}$ is universal: decide whether $s$ is eventually periodic, then apply the displayed equivalence.
